...
class App : PApplet() {
    ...
    private val globalScale = 400f
    private val col1 = color(255f, 30f)
    private val col2 = color(0f, 30f)
    ...
    fun getT(): Float {
        ...
        return (if (t < 1.0) t else 2.0f-t).let {
            Function.easeInOutCubic(it).toFloat()
        }
    }
        ...
        fun draw(t: Float) {
            if (parent is Node) {
                ...
                (pos * (1 - t) + parent.pos * t).let {
                    val _t = if (frameCount%duration/duration*2f < 1.0) {
                        t
                    } else {
                        2f-t
                    }
